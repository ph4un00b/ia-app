...
class LolaResponse {
  static Future<LolaTextResult> query({
    ...
    if (AppStatus.isOnboarding()) {
      const message = "Hola! Para poder usar mi inteligencia primero debes crear un recordatorio."
          " Cuando grabes un mensaje puedes mencionar algo como: 'Recuerdame preparar yogurt los sabados por la mañana.'";

      return switch ((userIntent.isConversational, AppStatus.instance.reminderStatus == ReminderState.idle)) {
        // (true, true) => LolaResult(
        //     p.normalize((await voiceModel.synthesize(text: message)).path),
        //     message,
        //   ),
        (true, true) => LolaTextResult(text: message),
        (true, false) => await OnboardingReminderHandler.handle(userQuery, voiceModel),
        (false, true) => await OnboardingReminderHandler.handle(userQuery, voiceModel),
        (false, false) => await OnboardingReminderHandler.handle(userQuery, voiceModel),
      };
    }

    final shouldHandleReminder =
        AppStatus.instance.lolaStatus == LolaState.creatingReminder || userIntent == IntentKind.createReminder;
